...
import com.ionic.helperfunctions.CaseClasses.{ FreqSketch, Histogram, HistogramAndPercentiles, HistogramAndStats, SimScore, Stats }
...
object Udfs {
  ...
  */
  ...
  // Reference: https://dev.ionic.com/sdk/formats/chunk
  val chunkCrptoV1Matcher = """~!([-_a-zA-Z0-9]+)~fEc!([a-zA-Z0-9+/]+={0,2})!cEf""".r
  val chunkCrptoV2Matcher = """~!2!([-_a-zA-Z0-9]+)!([a-zA-Z0-9+/]+={0,2})!""".r
  val chunkCrptoV3Matcher = """~!3!([-_a-zA-Z0-9]+)!([a-zA-Z0-9+/]+={0,2})!""".r
  val chunkCrptoV4Matcher = """~\\([-_a-zA-Z0-9]+);([a-zA-Z0-9+/]+={0,2})\\""".r

  // Get the attribute type given by the attribute-name
  def get_attribute_type(key: String, value: String): String = {
    if (key.startsWith("ionic-protected-") || key == "ionic-integrity-hash")
      "enc:ionic-protected"
    else {
      value match {
        case chunkCrptoV1Matcher(keytag, c) => "enc:ionic-encrypted:chunkv1"
        case chunkCrptoV2Matcher(keytag, c) => "enc:ionic-encrypted:chunkv2"
        case chunkCrptoV3Matcher(keytag, c) => "enc:ionic-encrypted:chunkv3"
        case chunkCrptoV4Matcher(keytag, c) => "enc:ionic-encrypted:chunkv4"
        case _ => "enc:plaintext"
      }
    }
  }
}
